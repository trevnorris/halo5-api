...
module.exports.getWarzoneServiceRecords = returnServiceRecordsFn('warzone');
module.exports.getCustomServiceRecords = returnServiceRecordsFn('custom');
module.exports.getCampaignServiceRecords = returnServiceRecordsFn('campaign');


function returnServiceRecordsFn(type) {
// If members is String then suspect it to be a single spartan.
  return function getServiceRecords(members, key, callback) {
    let ret_array = [];
    ...
    for (let i = 0; i < members.length; i++) {
      ...
      requests[i + 1] = setTimeout(function getData() {
        const req = util.httpsGetter({
          ...
          path: `/stats/h5/servicerecords/${type}?players=${query_members}`,
          headers: {
