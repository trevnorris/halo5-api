...
exports.httpsGetter = httpsGetter;
exports.generalizedStatGetter = generalizedStatGetter;
exports.resolveKey = resolveKey;
exports.validateData = validateData;
exports.callWithError = callWithError;
...
function generalizedStatGetter(key, api_path, callback) {
  (function generalGetter(tries) {
    httpsGetter({
      protocol: 'https:',
      ...
      if (err) {
        ...
        if (statusCode !== 429 && tries < util.maxRetries())
          return callback(err);

        // Too many requests made, try again later.
        return setTimeout(generalGetter, key.delay(), tries);
      }
...
function resolveKey() {
  ...
  let key = process.env.HALO5_DEV_KEY;
...
// Validate that incoming data is JSON. Then call callback appropriately.
